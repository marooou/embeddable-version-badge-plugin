package org.jenkinsci.plugins.versionbadge.RunBadgeAction
...
l.layout {
    l.main_panel {
        h2(_("Embeddable Version Badge Icon"))
        p(raw(_("blurb")))
        raw("""
<p>
</p>
...
""")
        ...
        def badge = base + "versionBadge/icon"
        
        def fullJobName = h.escape(my.project.fullName);
        def publicbadge = "${app.rootUrl}versionBadge/icon?job=${fullJobName}&build=${my.run.number}";
        h3 {
            text(_("Image"))
        ...
        h3(_("Markdown"))
        b {text(_("protected"))}
        input(type:"text",value:"[![Version Status](${badge})](${base})",class:"select-all")
        b {text(_("unprotected"))}
        input(type:"text",value:"[![Version Status](${publicbadge})](${base})",class:"select-all")
